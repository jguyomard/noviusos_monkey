...
class Controller_Front extends Controller_Front_Application {
    ...
    static function url_model($item, $first = false) {
        $model = get_class($item);
        if (in_array($model, array('Nos\Monkey\Model_Monkey', 'Nos\Monkey\Model_Species'))) {
            return \Nos\Tools_Enhancer::url_item('noviusos_monkey', array(get_called_class(), 'get_url_model'), $item, $first);
        }
    }
}
